...
maize_de <- data.frame(lapply(maize_fang_et_al_genotypes, as.character), stringsAsFactors=FALSE)
maize_de <- sapply(maize_de,function(x) {x <- gsub("?","-",x,fixed=TRUE)})

teosinte_de <- data.frame(lapply(teosinte_fang_et_al_genotypes, as.character), stringsAsFactors=FALSE)
teosinte_de <- sapply(teosinte_de,function(x) {x <- gsub("?","-",x,fixed=TRUE)})
...
for (i in 1:10) {
  maize <- filter(maize_de, Chromosome == i)
...
for (i in 1:10) {
  teosinte <- filter(teosinte_de, Chromosome == i)
